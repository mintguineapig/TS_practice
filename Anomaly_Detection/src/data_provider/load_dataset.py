...
def load_dataset(datadir: str,
    ...
    # train_test_split으로 validation set 분할 (80% train, 20% validation)
    ...
    if time_embedding and len(time_embedding) > 0:
        try:
            # 타임스탬프를 datetime으로 변환 (여러 방법 시도)
            # 방법 1: 기본 epoch time으로 시도
            ...
            try:
                ...
                try:
                    ...
                    # 모든 변환 실패시 더미 시간 특성 생성
                    trn_time_features = np.zeros((len(trn), 1))
                    val_time_features = np.zeros((len(val), 1))
                    tst_time_features = np.zeros((len(tst_data), 1))
        ...
        if hasattr(trn_time_features, 'values'):
            trn_time_features = trn_time_features.values
            val_time_features = val_time_features.values
            tst_time_features = tst_time_features.values
        
        # 지정된 시간 특성만 선택
        max_features = trn_time_features.shape[1]
        
        # time_embedding에서 정수 인덱스만 추출
        valid_indices = []
        for item in time_embedding:
            if isinstance(item, int) and 0 <= item < max_features:
